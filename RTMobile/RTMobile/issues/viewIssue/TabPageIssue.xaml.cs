    ...
    public partial class TabPageIssue : TabbedPage
    {
		...
		public TabPageIssue(Issue issues)
        {
            ...
            if (issues != null)
            {
                ...
                {
                    urlRequest = new Uri($"/rest/api/2/issue/{issues.key}?fields=*all"),
                    methodRequest = "GET"
                };
			...
			try
			{
				...
				{
					urlRequest = new Uri($"/rest/api/2/issue/{issues.key}/transitions/"),
					methodRequest = "GET"
				};
				...
				for (int i = 0; i < transition.Count; ++i)
				{
					...
					{
						await Navigation.PushAsync(new RTMobile.issues.viewIssue.Transition(int.Parse(transition[((ToolbarItem)sender).Priority - 1].id), issues.key)).ConfigureAwait(true);
					};
					ToolbarItems.Add(tb);
				}
			}
